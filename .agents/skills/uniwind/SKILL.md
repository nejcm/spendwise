...
9. **All theme variants must define the same set of CSS variables** — If `light` defines `--color-red`, then `dark` and every custom theme must too. Mismatched variables cause runtime errors.
...
const button = tv({
  base: 'items-center justify-center rounded-lg px-4 py-2 font-semibold',
  variants: {
    color: {
      primary: 'bg-blue-500 text-white',
        ...
        {themes.map((t) => (
          <Pressable
            ...
            className={`items-center rounded-lg px-4 py-3 ${
              activeTheme === t.name ? 'bg-primary' : 'border border-border bg-card'
            }`}
          >
...
```tsx
<View className="rounded-lg bg-linear-to-r from-red-500 via-yellow-500 to-green-500 p-4">
...
<View className="py-safe">{/* content */}</View>;
```
