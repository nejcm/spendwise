...
- **SQLite query functions** — import from `features/*/queries.ts` directly and run against in-memory SQLite via `createTestDb()` from `@/test-utils/sqlite-db`; no React context or mocking needed
