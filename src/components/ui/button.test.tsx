...
describe('button component ', () => {
  ...
  it('should call onClick handler when clicked', async () => {
    ...
    const { user } = setup(<Button testID="button" label="Click the button" onPress={onClick} />);
    expect(screen.getByTestId('button')).toBeOnTheScreen();
  ...
  it('should be disabled when loading', async () => {
    const onClick = jest.fn();
    const { user } = setup(<Button testID="button" loading={true} label="Click the button" onPress={onClick} />);
    expect(screen.getByTestId('button')).toBeOnTheScreen();
    expect(screen.getByTestId('button-activity-indicator')).toBeOnTheScreen();
  ...
  it('shouldn\'t call onClick when disabled', async () => {
    const onClick = jest.fn();
    const { user } = setup(
      <Button testID="button" label="Click the button" disabled={true} onPress={onClick} variant="secondary" />,
    );
    expect(screen.getByTestId('button')).toBeOnTheScreen();
    await user.press(screen.getByTestId('button'));
  ...
  it('should apply correct styles based on size prop', () => {
    ...
    const expectedStyle = 'font-family-sans font-semibold text-white dark:text-black text-xl';
  ...
  it('should apply correct styles for label when variant is secondary', () => {
    ...
    const expectedStyle = 'font-family-sans font-semibold text-secondary-600 text-base';
    const receivedStyle = button.props.children[0].props.children.props.className;
    expect(receivedStyle).toContain(expectedStyle);
  });
  it('should apply correct styles for label when is disabled', () => {
    ...
    const expectedStyle = 'font-family-sans font-semibold text-base text-neutral-600 dark:text-neutral-600';
    const receivedStyle = button.props.children[0].props.children.props.className;
    expect(receivedStyle).toContain(expectedStyle);
  });
});
