...
describe('getGlobalBudget', () => {
  ...
  it('returns the stored integer value in cents', async () => {
    const db = await createTestDb();
    await setGlobalBudget(db as any, 300_000);
    expect(await getGlobalBudget(db as any)).toBe(300_000);
  });

  it('returns null after budget is cleared', async () => {
    const db = await createTestDb();
    await setGlobalBudget(db as any, 100_000);
    await setGlobalBudget(db as any, null);
    expect(await getGlobalBudget(db as any)).toBeNull();
  });

  it('returns null for a malformed non-numeric value in _meta', async () => {
    const db = await createTestDb();
    await db.runAsync(
  ...
  it('overwrites an existing value on update', async () => {
    const db = await createTestDb();
    await setGlobalBudget(db as any, 100_000);
    await setGlobalBudget(db as any, 250_000);
    expect(await getGlobalBudget(db as any)).toBe(250_000);
  });
});
